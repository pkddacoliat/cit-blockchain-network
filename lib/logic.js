...
async function assignGrade(gradeData) {
  ...
  if (!moduleExists) {
    ...
    // Get the student registry
    let studentRegistry = await getParticipantRegistry("ie.cit.blockchain.participant.Student");
    ...
    if (!studentExists) {
      ...
      if (gradeExists) {
        ...
        let grade = factory.newResource("ie.cit.blockchain.grade", "Grade", gradeId);
        ...
        let studentInstance = await studentRegistry.get(gradeData.studentNumber);
